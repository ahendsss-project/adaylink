<div x-data="{ showGallery: false, showTranslation: {{ $multilanguageEnabled ? 'true' : 'false' }} }">
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-5 md:p-6">
        ...
        <form wire:submit="save" class="space-y-5">
            ...
            {{-- Thumbnail with URL / Upload / Gallery --}}
            <div>
                <label class="block text-sm font-medium text-gray-600 mb-1">Thumbnail / Gambar Utama</label>
                <x-media-input
                    url-model="thumbnail_url"
                    file-model="thumbnail_file"
                    placeholder="https://example.com/foto-tour.jpg"
                    preview-class="w-48 h-32 object-cover"
                    :preview-url="$thumbnail_file ? $thumbnail_file->temporaryUrl() : ($thumbnail_url ?: null)"
                >
                    <x-slot:url-extra>
